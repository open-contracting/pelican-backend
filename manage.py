...
def markdown_category(messages):
...
def markdown_page():
    ...
    for title, description_key, function, checks_key, defined in (
        ("Field-level", "field", markdown_category, "fieldDetail", field_identifiers),
        ("Compiled release-level", "resourceLevel", markdown_category, "resourceLevel", set(resource_level)),
        ("Dataset-level", "datasetLevel", markdown_category, "datasetLevel", set(dataset_level)),
        ("Time-based", "timeLevel", functools.partial(markdown_checks, level=3), "timeLevel", set(time_level)),
    ):
