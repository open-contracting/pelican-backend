...
def do_work(dataset_id):
    ...
    while processed_count == page_size:
        ...
        for item in items:
            ...
            for plugin_name, plugin in definitions.items():
                logger.log(CustomLogLevels.CHECK_TRACE,
                ...
                try:
                    filtering_result = plugin.filter(scope[plugin_name],
                                                     ancestor_item,
                                                     ancestor_item_id,
                                                     new_item,
                                                     new_item_id)
    ...
    if no_item_processed:
        logger.info(
            "No item with dataset_id {} found. Skipping time variance checks computation.".format(dataset_id)
        )

    for plugin_name, plugin in definitions.items():
        logger.info(
            "Getting result for {} time variance check.".format(plugin_name)
        )
