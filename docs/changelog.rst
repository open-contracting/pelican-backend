...
-  feat: ``misc.url_availability`` samples unique URL values, so that a repeated URL neither dominates the sample nor is requested more than once, and skips if fewer than 100 unique URL values are present. Bump its ``version`` to 2.0. Also, close responses. :issue:`6`
