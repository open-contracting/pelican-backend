...
This project opens one connection per thread, so that one thread doesn't commit another's incomplete work. Each :doc:`worker<workers>` uses one thread, except the :ref:`check-data-item` worker, which uses ``DATA_ITEM_PREFETCH_COUNT`` threads (one per CPU, by default), and the :ref:`check-dataset` worker, which uses ``DATASET_PREFETCH_COUNT`` threads (2, by default, since each thread holds the state of all dataset-level checks for one dataset in memory, which grows with dataset size). Set these to keep total connections within PostgreSQL's ``max_connections``, taking into account other workers and services.
