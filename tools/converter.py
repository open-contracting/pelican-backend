...
def import_data(data):
    ...
    for value in rates.values():
        currencies.update(value.keys())

    if get_param('interpolation'):
        ...
        for currency, dates in real_data_dates.items():
            ...
            for date in dates:
                if (date - previous).days > 1:
                    if get_param('interpolation') 'closest':
                        interpolation_closest(currency, previous, date)
                    elif get_param('interpolation') == 'linear':
...
def interpolation_closest(currency, start_date, end_date):
    ...
    while current_date < end_date:
        ...
        if get_param('interpolation_max_days_fallback') != -1 and distance_to_start > get_param('interpolation_max_days_fallback') \
                and distance_to_end > get_param('interpolation_max_days_fallback'):
            current_date += timedelta(days=distance_to_end - get_param('interpolation_max_days_fallback'))
            continue
...
def interpolation_linear(currency, start_date, end_date):
    ...
    while current_date < end_date:
        ...
        if get_param('interpolation_max_days_fallback') != -1 and \
                distance_to_start > get_param('interpolation_max_days_fallback') \
                and distance_to_end > get_param('interpolation_max_days_fallback'):
            current_date += timedelta(days=distance_to_end - get_param('interpolation_max_days_fallback'))
            continue
        else:
...
def extrapolation_closest_rate(currency, rel_date):
        ...
        if bound[0] > rel_date and (bound[0] - rel_date).days <= get_param('extrapolation_max_days_fallback'):
            return rates[bound[0]][currency]
        elif bound[1] < rel_date and (rel_date - bound[1]).days <= get_param('extrapolation_max_days_fallback'):
...
def convert(amount, original_currency, target_currency, rel_date):
    ...
    elif extrapolation_type == 'closest':
