...
def test_prices():
    ...
    assert result["compiled_releases"]["total_unique_ocids"] == 1
    assert result["tender_lifecycle"] == {"planning": 0, "tender": 0, "award": 0, "contract": 15, "implementation": 0}
    ...
    assert result["prices"] == {
        ...
        "price_category_positive": {
            "0_10000": {"contracts": 3, "volume": 3 * 2000, "share": 3 * 2000 / total_volume_positive},
            "10001_100000": {"contracts": 3, "volume": 3 * 20000, "share": 3 * 20000 / total_volume_positive},
            "100001_1000000": {"contracts": 3, "volume": 3 * 200000, "share": 3 * 200000 / total_volume_positive},
            "1000001+": {"contracts": 3, "volume": 3 * 2000000, "share": 3 * 2000000 / total_volume_positive},
        },
    }
    assert result["period"] == [{"date_str": "Jan-19", "count": 5}]
